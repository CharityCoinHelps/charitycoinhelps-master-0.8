...
uint256 hashGenesisBlock("0xd7fe1ae5953b474ee25d80c0937624643e9717383b9b6f36dba9af6002203b53");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // CharityCoinHelps: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "CharityCoinHelps Signed Message:\n";
...
{
    // CharityCoinHelps: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // CharityCoinHelps: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // CharityCoinHelps: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // CharityCoinHelps: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // CharityCoinHelps: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // CharityCoinHelps: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // CharityCoinHelps: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xa5f939f81e3cf836c8324202e0cd7f982087dfaa512cf223b4159dd86f96cc3a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "PLEASE_REQUEST"; // to create a mining pool please request timestamp: http://www.charitycoinhelps.org/index.php/more-info
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 0; // to create a mining pool please request nTime: http://www.charitycoinhelps.org/index.php/more-info
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 0; // to create a mining pool please request nNonce: http://www.charitycoinhelps.org/index.php/more-info

        if (fTestNet)
        {
            block.nTime    = 1392372179; // Feb 14
            block.nNonce   = 1010488;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xe03c239c0c6500fbfa7d3ab5e27d50fcc50fc02039af2880753a93051eebfa2b"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // CharityCoinHelps: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CharitycoinhelpsMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CharitycoinhelpsMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CharitycoinhelpsMiner(CWallet *pwallet)
{
    printf("CharitycoinhelpsMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("charitycoinhelps-miner");
    ...
    try { loop {
        ...
        printf("Running CharitycoinhelpsMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CharitycoinhelpsMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CharitycoinhelpsMiner, pwallet));
}
